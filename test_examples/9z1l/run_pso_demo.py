"""
Kinematic Particle Swarm Optimization (Kin-PSO) on PDB 9Z1L (KIT V654A + BLU-654/A1CZZ).
Runs a 20-particle swarm over BLU-654's exocyclic torsion + rigid-body space
to find the best-scoring pose, and exports the full multi-particle
...
print("=" * 80)
print("  OPENMM-DOCK: KINEMATIC PARTICLE SWARM OPTIMIZATION (KIN-PSO) -- PDB 9Z1L")
print("=" * 80)
...
xtal_coords = lig_mol.GetConformer().GetPositions()
best_coords = best_mol.GetConformer().GetPositions()
best_rmsd = float(np.sqrt(np.mean(np.sum((best_coords - xtal_coords) ** 2, axis=1))))
print(f"[*] Best Pose RMSD to Crystal: {best_rmsd:.2f} Å")
...
pml_content = f"""# PyMOL Script for Kinematic Particle Swarm Optimization Movie (PDB 9Z1L)
...
print "================================================================="
print "  Loaded {len(swarm_frames)}-frame Kinematic Particle Swarm (Kin-PSO) Movie!"
