...
Unifies:
1. Macrocycle Inverse Kinematics (IK Loop Closure on Ring)
...
4. Particle Swarm Optimization (PSO on Coupled SE(3) x T^k Manifold)
"""
...
class UnifiedSwarmParticle:
    ...
    ring_driver_angles: np.ndarray # (2,) Ring IK driver dihedrals
...
class UnifiedKinematicPSOEngine:
    """
    Coordinates coupled Particle Swarm Optimization across macrocycle ring (IK),
    ...
    def __init__(
        ...
        print(f"[*] Unified Kinematic PSO Initialized:")
        print(f"    • Macrocycle: 10 Ring IK Joints (2 Drivers) + {self.num_exo} Exocyclic FK Joints")
        print(f"    • Receptor  : {len(self.rec_kin.flex_residues)} Pocket Residues ({self.num_rec_chi} Chi Joints)")
        print(f"    • Total Degrees of Freedom: {6 + self.num_ring_drivers + self.num_exo + self.num_rec_chi}")
    ...
    def evaluate_coupled_state(
        ...
        1. Solves Macrocycle IK loop closure
        2. Rotates Exocyclic Ligand Arms
        3. Rotates Receptor Active-Site Side Chains
        ...
        d_dict = {1: float(ring_drivers[0]), 5: float(ring_drivers[1])}
        c_lig, _, _ = self.two_tier_lig.ik_engine.solve_loop_closure(
            self.two_tier_lig.base_coords, driver_angles=d_dict
        )
        ...
        # Set full positions (receptor + ligand + solvent)
        full_pos = self.engine._full_positions_from_coords(c_lig)
        # Overwrite receptor coordinates with articulated receptor coordinates
