...
class BridgedTwoStageDockingEngine:
    ...
    def __init__(
        ...
        # Initialize Stage 1 (Rigid Receptor) and Stage 2 (Flexible Receptor) engines
    ...
    def run_bridged_docking_pipeline(
        ...
        # =====================================================================
        # STAGE 1: GLOBAL SWARM INGRESS (Rigid Receptor)
        # =====================================================================
        print("\n[>>>] STAGE 1: Launching Global Swarm Ingress (Rigid Receptor)...")
        ...
        # =====================================================================
        # STAGE 2: IN-POCKET KINEMATIC INDUCED-FIT RELAXATION
        # =====================================================================
        print("\n[>>>] STAGE 2: Launching In-Pocket Kinematic Induced-Fit Refinement...")
        print(f"    • Unlocking {len(self.engine_stage2.rec_kin.flex_residues)} Active-Site Residues ({self.engine_stage2.num_rec_chi} Chi Joints)")
        print(f"    • Unlocking Ligand Kinematics ({self.engine_stage2.num_exo} Rotatable Arms)")
        ...
        # Multi-angle rotational sweep + Kinematic relaxation
        angles = [0, 90, 180, 270]
        ...
        for ang in angles:
            for flip in [False, True]:
                ...
                # Apply IK if macrocyclic
                if len(two_tier.ik_engine.joints) >= 4:
                    c_ik, _, _ = two_tier.ik_engine.solve_loop_closure(cand_rot, driver_angles={1: 0.1, 5: -0.1})
                else:
                    c_ik = cand_rot.copy()
        ...
        # Final Analytical Polish on Best Stage 2 State
    ...
    def plot_stage_transition(
        ...
        ax1.scatter(np.array(frames)[p1_idx], np.array(rmsds)[p1_idx], color="coral", s=12, alpha=0.6, label="Stage 1: Global Swarm Ingress (Rigid)")
        ax1.scatter(np.array(frames)[p2_idx], np.array(rmsds)[p2_idx], color="mediumseagreen", s=18, alpha=0.8, label="Stage 2: Induced-Fit Plasticity")
        ...
        ax2.scatter(np.array(frames)[p1_idx], np.clip(np.array(scores)[p1_idx], -500, 2000), color="coral", s=12, alpha=0.6)
        ax2.scatter(np.array(frames)[p2_idx], np.clip(np.array(scores)[p2_idx], -500, 2000), color="mediumseagreen", s=18, alpha=0.8)
